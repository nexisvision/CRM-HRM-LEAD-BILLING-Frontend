...
const getnote = async (id) => {
  ...
  try {
    const res = await axios.get(`http://localhost:5353/api/v1/notes/${id}`, {
      headers: {
...
const addnote = async (id, formData) => {
  ...
  try {
    const res = await axios.post(
      `http://localhost:5353/api/v1/notes/${id}`,
      formData,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
const adddelnote = async (id) => {
  ...
  try {
    const res = await axios.delete(`http://localhost:5353/api/v1/notes/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
const editelnote = async (idd, formData) => {
  ...
  try {
    const res = await axios.put(
      `http://localhost:5353/api/v1/notes/${idd}`,
      formData,
      {
        headers: {
          Authorization: `Bearer ${token}`,
