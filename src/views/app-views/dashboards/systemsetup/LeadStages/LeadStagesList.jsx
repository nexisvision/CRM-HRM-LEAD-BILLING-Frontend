...
const LeadStagesList = () => {
  ...
  useEffect(() => {
    if (fnddata) {
      const filteredData = fnddata.filter((item) => item.stageType === "lead");
  ...
  const handlePipelineChange = (value) => {
    ...
    if (value === "all") {
      setLeadadatafilter(fnddata.filter((item) => item.stageType === "lead"));
    } else {
      const filtered = fnddata.filter(
        (item) => item.stageType === "lead" && item.pipeline === value
      );
  ...
  return (
    <>
      ...
      <Modal
        ...
        className="mt-[-70px]"
      >
        <EditLeadStages onClose={closeEditLeadStagesModal} idd={idd} />
      </Modal>
    </>
  );
};
