...
const InvoiceService = {
  ...
  getAllInvoices: async (id) => {
    ...
    try {
        ...
        const response = await axios.get(`http://localhost:5353/api/v1/invoices/${id}`, {
            headers: {
  ...
  getMilestoneDetails: async (id) => {
    const token = localStorage.getItem('auth_token');
    try {
      const response = await axios.get(`http://localhost:5353/api/v1/milestones/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  updateInvoice: async (id,data) => {
    const token = localStorage.getItem('auth_token');
    try {
      const response = await axios.put(`http://localhost:5353/api/v1/invoices/${id}`, data, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
...
deleteInvoice: async (id) => {
  const token = localStorage.getItem('auth_token');
  try {
      const response = await axios.delete(`http://localhost:5353/api/v1/invoices/${id}`, {
          headers: {
              Authorization: `Bearer ${token}`,
              'Content-Type': 'application/json'
          }
