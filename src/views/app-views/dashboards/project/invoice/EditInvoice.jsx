...
const EditInvoice = ({ idd, onClose, setFieldValue, values }) => {
  ...
  const curren = useMemo(() => currenciesState?.currencies?.data || [], [currenciesState?.currencies?.data]);
    ...
    if (value) {
      ...
      if (selectedMile) {
        const newTableData = [{
          ...
          quantity: 1,
          price: selectedMile.milestone_cost || 0,
          tax: 0,
          discountType: "percentage",
          discount: 0,
  ...
  const handleDeleteRow = (id) => {
    if (tableData.length > 1) {
      ...
      calculateTotal(updatedTableData, discountRate);
    } else {
        ...
        <React.Fragment key={row.id}>
          ...
          <tr>
            <td colSpan={8} className="px-2 py-2 border-b">
              <textarea
                rows={2}
                value={row.description ? row.description.replace(/<[^>]*>/g, '') : ''}
