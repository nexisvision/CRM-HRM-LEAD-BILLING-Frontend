...
const STATUS_CONFIG = {
	'Incomplete': { color: '#ff4d4f', icon: <CloseCircleOutlined style={{ color: '#ff4d4f' }} /> },
	'To Do': { color: '#1890ff', icon: <ClockCircleOutlined style={{ color: '#1890ff' }} /> },
	'In Progress': { color: '#faad14', icon: <ClockCircleOutlined style={{ color: '#faad14' }} /> },
	'Completed': { color: '#52c41a', icon: <CheckCircleOutlined style={{ color: '#52c41a' }} /> },
	'On Hold': { color: '#d9d9d9', icon: <ExclamationCircleOutlined style={{ color: '#d9d9d9' }} /> }
};
				...
				if (item.project_members) {
					if (typeof item.project_members === 'string') {
						try {
							...
							projectMembers = Array.isArray(parsed) ? parsed :
								parsed.project_members ? parsed.project_members : [];
						} catch (e) {
							console.error('Error parsing project members string:', e);
							projectMembers = [];
						}
					} else if (typeof item.project_members === 'object') {
						projectMembers = Array.isArray(item.project_members) ? item.project_members :
							item.project_members.project_members ? item.project_members.project_members : [];
					}
				}
						...
						{statuses.map(statusOption => (
							<Select.Option key={statusOption.id} value={statusOption.name}>
								<div className="flex items-center">
									<span className="h-2 w-2 rounded-full mr-2" style={{ backgroundColor: STATUS_CONFIG[statusOption.name]?.color || '#d9d9d9' }} />
									{statusOption.name}
								</div>
							</Select.Option>
						))}
					...
					>
						{tags.map(tagOption => (
							<Select.Option key={tagOption.id} value={tagOption.name}>
								<div className="flex items-center">
									<span className="h-2 w-2 rounded-full mr-2" style={{ backgroundColor: TAG_COLORS[tagOption.name.toLowerCase()] || '#d9d9d9' }} />
									{tagOption.name}
								</div>
							</Select.Option>
						))}
	...
	const handleStatusChange = async (projectId, newStatus) => {
		try {
			...
			try {
				...
				} else if (originalProject.project_members && originalProject.project_members.project_members) {
										...
										>
											...
											<div className="p-4">
												...
												<div className="flex justify-between items-start mb-4">
													...
													<Dropdown overlay={dropdownMenu(item.id)} trigger={['click']} placement="bottomRight">
														<Button
															type="text"
												...
												{/* Project Members Row */}
												<div className="mb-4">
													<p className="text-xs text-gray-500 mb-2">Project Members</p>
													<UserAvatarGroup users={item.memberDetails} maxCount={3} />
												</div>
													...
													<div>
														...
														<div className="flex items-center">
															<span
																...
																style={{ backgroundColor: STATUS_CONFIG[item.status]?.color || '#d9d9d9' }}
															/>
															<span className="text-sm">{item.status || 'Not Started'}</span>
														</div>
													</div>
													<div>
														...
														<div className="flex items-center">
															<span
																className="h-2 w-2 rounded-full mr-2"
																style={{ backgroundColor: TAG_COLORS[item.tag?.toLowerCase()] || '#d9d9d9' }}
															/>
															<span className="text-sm">{item.tag || 'No Tag'}</span>
														</div>
													</div>
												</div>
												...
												<div className="grid grid-cols-2 gap-3 mb-4">
													...
													<div className="flex items-center p-2.5 bg-gray-50 rounded-lg">
														<ClockCircleOutlined
															className="mr-2"
															style={{ color: item.dayleft > 10 ? '#52c41a' : item.dayleft > 5 ? '#faad14' : '#f5222d' }}
														/>
														<div>
															<p className="text-xs text-gray-500 m-0">Days Left</p>
															<p className="text-sm font-semibold m-0">{item.dayleft} days</p>
														</div>
													</div>
												</div>
